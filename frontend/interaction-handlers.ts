...
function selectEdge(
  ...
  g.selectAll('.link')
    ...
    .attr('stroke-width', d =>
      d === edgeData
        ? Math.max(4, ((d as D3SimulationLink).thickness || calculateEdgeWeight(d)) * 1.8)
        : (d as D3SimulationLink).thickness || calculateEdgeWeight(d)
    );
  ...
  g.selectAll('.node circle')
    ...
    .ease(d3.easeQuadOut)
    .attr('stroke', d =>
      (d as GraphNode).id === (edgeData.source as GraphNode).id ||
      (d as GraphNode).id === (edgeData.target as GraphNode).id
        ? '#e74c3c'
        : '#fff'
    )
    .attr('stroke-width', d =>
...
function createNodeHoverHandlers(showNodeTooltip, hideNodeTooltip) {
  const mouseenter = function (event, d) {
    ...
    d3.select(this).select('circle').attr('stroke-width', 4).attr('stroke', '#f39c12');
    ...
    g.selectAll('.link')
      .attr('stroke-opacity', linkData => {
        ...
        const isConnected =
          (linkData as D3SimulationLink).source.id === d.id ||
          (linkData as D3SimulationLink).target.id === d.id;
    ...
    g.selectAll('.node')
      ...
      .attr('opacity', nodeData => {
        ...
        const isConnected =
          d.imports.includes((nodeData as GraphNode).id) ||
  ...
  const mouseleave = function (event, d) {
    ...
    g.selectAll('.link')
      ...
      .attr(
        'stroke-width',
        linkData => (linkData as D3SimulationLink).thickness || calculateEdgeWeight(linkData)
      );

    // Reset node opacity
    g.selectAll('.node').select('circle').attr('opacity', 1);
...
function highlightSearchResults(searchTerm) {
  ...
  g.selectAll('.node circle')
    .attr('stroke', d => {
      ...
      const matches =
        fileName.includes(searchTerm) || (d as GraphNode).path.toLowerCase().includes(searchTerm);
