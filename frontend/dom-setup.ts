...
function initializeControlHandlers(
  ...
  if (collapseImported)
    collapseImported.addEventListener('click', () =>
      togglePanel('most-imported-panel', 'collapse-imported')
    );
  if (collapseExports)
    collapseExports.addEventListener('click', () =>
      togglePanel('most-exports-panel', 'collapse-exports')
    );
}
