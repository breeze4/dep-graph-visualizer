...
import { transformGraphData } from './graph-transformer.ts';
...
import { applyFilters, toggleNodeLabels, updateMinLoc, resetFilters } from './filters.ts';
...
import { updateGraphStatistics } from './stats-ui.ts';
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const keyboardState = initializeKeyboardHandlers(updateMultiSelectIndicator, clearMultiSelection);

  // Create wrapped functions with necessary context
  const wrappedValidateAndLoadGraph = (data, fromCache = false, cacheKey = null) => {
    const result = validateAndLoadGraph(
      data,
      fromCache,
      cacheKey,
  ...
  function initializeVisualizationWhenReady() {
    if (currentGraphData) {
      ...
      setTimeout(() => {
        ...
        const nodeHoverHandlers = createNodeHoverHandlers(showNodeTooltip, hideNodeTooltip);
