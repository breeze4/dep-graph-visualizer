...
function enterFocusMode(nodeId) {
  ...
  g.selectAll('.link')
    ...
    .style('opacity', d => {
      ...
      return connectedNodeIds.has(linkData.source.id) && connectedNodeIds.has(linkData.target.id)
        ? 0.7
        : 0.05;
    })
    .on('end', function (d) {
      ...
      const linkData = d as D3SimulationLink;
      const isConnected =
        connectedNodeIds.has(linkData.source.id) && connectedNodeIds.has(linkData.target.id);
...
function showInterfaceExtractionPanel() {
  ...
  if (selectedNodesList.length < 2) {
    document.getElementById('file-info').innerHTML =
      '<p>Select at least 2 nodes for interface extraction</p>';
    return;
  }
  ...
  fileInfo.innerHTML = `
        <div class="interface-extraction">
            ...
            <div class="selected-modules">
                ${selectedNodesList
                  .map(node => `<span class="module-tag">${node.name}</span>`)
                  .join('')}
            </div>
...
function enterConnectionFocus(sourceId, targetId) {
  ...
  g.selectAll('.node')
    ...
    .on('end', function (d) {
      d3.select(this).style(
        'pointer-events',
        connectedNodeIds.has((d as GraphNode).id) ? 'all' : 'none'
      );
    });
  ...
  g.selectAll('.link')
    ...
    .ease(d3.easeQuadInOut)
    .style('opacity', d =>
      connectedNodeIds.has((d as D3SimulationLink).source.id) &&
      connectedNodeIds.has((d as D3SimulationLink).target.id)
        ? 0.7
        : 0.05
    )
    .on('end', function (d) {
...
function clearAllHighlights() {
  ...
  g.selectAll('.link').transition().duration(300).ease(d3.easeQuadOut).attr('stroke-opacity', 0.7);

  g.selectAll('.node circle').transition().duration(300).ease(d3.easeQuadOut).attr('opacity', 1);
...
function updateEdgeActionButtons(sourceId, targetId) {
  ...
  const highlightBtn = document.querySelector(
    `#highlight-btn[data-source="${sourceId}"][data-target="${targetId}"]`
  );
  const focusBtn = document.querySelector(
    `#focus-btn[data-source="${sourceId}"][data-target="${targetId}"]`
  );
...
const setMultiSelectMode = value => {
  multiSelectMode = value;
};
const setIsShiftPressed = value => {
